    ...
    **/
    ...
    private ArrayList<Rating> getSimilarities(String id) {
        ...
        for (Rater r : RaterDatabase.getRaters()) {
            ...
            if (!me.getID().equals(r.getID())) {
                double similarityVal = dotProduct(me, r);
                if (similarityVal >= 0) {
                /** 
    ...
    public ArrayList<Rating> getSimilarRatings(String id, int numSimilarRaters, int minimalRaters) {
        ...
        for (String movieId : MovieDatabase.filterBy(new TrueFilter())) {
            ...
            double wAvg = 0;
            double raterCount = 0;
            //System.out.println(numSimilarRaters);
            //System.out.println(similarityRatings.size());
            // for each rater based on top numSimilarRater
            for (int k = 0; k < numSimilarRaters; k++) {
                // get each similarityRating by index
                //System.out.println(k + "\t" + numSimilarRaters);
                Rating similarityRating = similarityRatings.get(k);
